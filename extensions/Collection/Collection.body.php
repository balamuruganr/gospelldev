...
class SpecialCollection extends SpecialPage {
	...
	function execute( $par ) {
		global $wgUser, $wgCollectionMaxArticles;
	...
	function renderBookCreatorPage( $referer, $par, $is_rename = '' ) {
       ...
       $output = '<div style="margin:10px 0;">
        ...
        $out->addHTML($output);           
		/*$out->addHTML( Click OK to continue with your book  which contains 2 wiki pages. Click Cancel to delete it and start with an empty book
			Xml::tags( 'div',
				array(
					'style' => 'margin: 10px 0;',
				),                
	 ...
	 * @return bool
	 */
	static function addArticleFromName( $namespace, $name, $oldid = 0, $book_id = 0 ) {
		$title = Title::makeTitleSafe( $namespace, $name );
		if ( !$title ) {
			return false;
		}
		return self::addArticle( $title, $oldid, $book_id );
	}
	 ...
	 * @return bool
	 */
	static function addArticle( $title, $oldid = 0, $book_id = 0 ) {
		...
		$index = CollectionSession::findArticle( $prefixedText, $oldid, $book_id );
		if ( $index != - 1 ) {
			return false;
		}
	 ...
	 * @return bool
	 */
	static function removeArticleFromName( $namespace, $name, $oldid = 0, $book_id = 0 ) {
		$title = Title::makeTitleSafe( $namespace, $name );
		return self::removeArticle( $title, $oldid, $book_id );
	}
	 ...
	 * @return bool
	 */
	static function removeArticle( $title, $oldid = 0, $book_id = 0 ) {
	   global $wgUser;
	...
	static function addCategory( $title ) {
		...
		foreach ( $res as $row ) {
			...
			if ( in_array( $row->page_namespace, $wgCollectionArticleNamespaces ) ) {
				...
				if ( CollectionSession::findArticle( $articleTitle->getPrefixedText(), 0, 0 ) == - 1 ) {
